...
trace_w <- trace[[1]] %>% 
  ...
  mutate(joint_p = case_when(`phi_w[1]` <= 0 & `sigma_w[1]` <= 0 ~ "both_negative",
                             TRUE ~ "one_or_both_positive"))
```
...
trace_e_alt <- trace[[1]] %>% 
  ...
  mutate(joint_p = case_when(`phi_e[2]` <= 0 & `sigma_e[2]` <= 0 ~ "both_negative",
                             TRUE ~ "one_or_both_positive"))
 
trace_b_alt <- trace[[1]] %>% 
  dplyr::select(Iteration, `phi_b[2]`, `sigma_b[2]`) %>% 
  mutate(joint_p = case_when(`phi_b[2]` <= 0 & `sigma_b[2]` <= 0 ~ "both_negative",
                             TRUE ~ "one_or_both_positive"))
 
trace_w_alt <- trace[[1]] %>% 
  dplyr::select(Iteration, `phi_w[2]`, `sigma_w[2]`) %>% 
  mutate(joint_p = case_when(`phi_w[2]` <= 0 & `sigma_w[2]` <= 0 ~ "both_negative",
                             TRUE ~ "one_or_both_positive"))
```
...
state_labels <- read.csv(here("biogeography/without_zerene/server_ignore/output/altmean/run2/state_labels.txt"), 
