...
log_dir <- "../../output/sim/rj/rj_output"
true_dir <- "../../data/sim/history"
summary_dir <- "../../output/sim/rj/rj_data"

files <- list.files(path=log_dir, pattern=paste("*", ".Y.model.log", sep=""), full.names=TRUE, recursive=FALSE)
params <- c("r_b","r_d","r_e","r_w","sigma_w[1]","sigma_w[2]","sigma_e[1]","sigma_e[2]","sigma_b[1]","sigma_b[2]","sigma_d[1]","sigma_d[2]","phi_w[1]","phi_w[2]","phi_e[1]","phi_e[2]","phi_b[1]","phi_b[2]","phi_d[1]","phi_d[2]","rj_sigma_w[1]","rj_sigma_w[2]","rj_sigma_e[1]","rj_sigma_e[2]","rj_sigma_b[1]","rj_sigma_b[2]","rj_sigma_d[1]","rj_sigma_d[2]","rj_phi_w[1]","rj_phi_w[2]","rj_phi_e[1]","rj_phi_e[2]","rj_phi_b[1]","rj_phi_b[2]","rj_phi_d[1]","rj_phi_d[2]")
...
for (file in files) {
  ...
  data <- read.csv(file=paste(log_dir, "/", sim, ".Y.model.log", sep=""), sep = "\t", header=TRUE)
